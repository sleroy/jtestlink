function ZoomableTreemapBuilder() {
	...
	this.build = function(data) {
		...
		var build_from_json = function(root) {
			...
			// Aggregate the values for internal nodes. This is
			// normally done by the
			// treemap layout, but not here because of our
			// custom implementation.
			// We also take a snapshot of the original children
			// (_children) to avoid
			// the children being overwritten when when layout
			// is computed.
			...
			// Compute the treemap layout recursively such that
			// each group of siblings
			// uses the same size (1×1) rather than the
			// dimensions of the parent cell.
			// This optimizes the layout for the current zoom
			// state. Note that a wrapper
			// object is created for the parent node for each
			// group of siblings so that
			// the parent’s dimensions are not discarded as we
			// recurse. Since each group
			// of sibling was laid out in 1×1, we must rescale
			// to fit using absolute
			...
			function display(d) {
				...
				function transition(d) {
					...
					// Enable anti-aliasing during the
					// transition.
					...
					// Remove the old node when the transition
					// is finished.
