...
class Gravatar {
	...
	GravatarDefaultImage defaultImage = DEFAULT_DEFAULT_IMAGE
	GravatarRating rating = DEFAULT_RATING
	 ...
	 * gravatar was found.
	 */
	def byte[] download(final String email) throws GravatarDownloadException {

		InputStream stream = null
		try {
			final URL url = new URL(this.getUrl(email))
	 ...
	 */
	def String getUrl(final String _email) {
		...
		final String emailHash = DigestUtils.md5DigestAsHex(emailString
				.toLowerCase().trim().getBytes())
		final String params = this.formatUrlParameters()
	 ...
	 */
	def void setDefaultImage(final GravatarDefaultImage defaultImage) {
	 ...
	 */
	def void setRating(final GravatarRating rating) {
	 ...
	 */
	def void setSize(final int sizeInPixels) {
	...
	def String formatUrlParameters() {
